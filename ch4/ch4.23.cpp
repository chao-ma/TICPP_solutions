...
void LineHolder::initialize(int size) {
	this->size = size;
}

void LineHolder::put_line(char** line) {
	if (stack.peek() == 0 || static_cast<Stash*>(stack.peek())->count() == 5) {
		Stash* p_stash = new Stash;
		p_stash->initialize(size);
		stack.push(p_stash);
	}
	static_cast<Stash*>(stack.peek())->add(line);
}

void LineHolder::print() {
	if (stack.peek() != 0) {
